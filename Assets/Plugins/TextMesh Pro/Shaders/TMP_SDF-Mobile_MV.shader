...
Shader "TextMeshPro/Mobile/Distance Field AppSW" {
...
SubShader {
	...
	Pass
	{
		...
		struct Varyings
		{
			float4 vertex : SV_POSITION; // custom space ? 
		...
		Varyings vert(Attributes input)
		{
			...
			float3 pos_os = input.vertex.xyz;
			pos_os.x += _VertexOffsetX;
			pos_os.y += _VertexOffsetY;
			VertexPositionInputs vertexInput = GetVertexPositionInputs(pos_os);

			output.vertex = vertexInput.positionCS;

			float3 curWS = TransformObjectToWorld(input.vertex.xyz);
